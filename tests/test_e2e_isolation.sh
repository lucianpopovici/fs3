...
#   9. SIGHUP key rotation racing an in-flight request (ASan/UBSan
...
#    use-after-free regression). sigv4_verify_id copies the matched
#    credential's user into conn_t.id at verify time (never a pointer
#    into the credential list — see src/sigv4.c's comment above the
#    snprintf in sigv4_verify_id), specifically so a rotation landing
#    mid-request can't touch freed memory. This section exercises that
#    race directly: most meaningful run under `DEBUG=1` (ASan+UBSan
#    instrumented fs3), but it still exercises the race path either way.
# ===================================================================
write_creds "$A3" "$A3S"
start_server --credentials-file "$CRED_FILE" --require-auth --admin root
check_eq "alice creates a bucket for the slow PUT" \
    "$(req $A3 $A3S --method PUT --url "$URL/alice-slow")" "STATUS=200"

coproc SLOWPUT {
    FS3_AK="$A3" FS3_SK="$A3S" \
...
if [ "$pause_line" != "PAUSED" ]; then
    ...
    # Rotate alice's key while the PUT is paused mid-body, then HUP.
    A4=AKIAALICE0000000004; A4S=alicesecret00000000000000000000004
    write_creds "$A4" "$A4S"
    kill -HUP "$SP"
    sleep 1.5
    if ! kill -0 "$SP" 2>/dev/null; then
